...
- **Real GIF progress** — because that single invocation has no per-file boundaries to count, `run_with_progress()` runs it under ImageMagick's `-monitor` via `Popen` and parses the progress it writes to stderr (`"<Op>/<Type>: <offset> of <extent>, <pct>% complete"`, carriage-return terminated). `make_gif_progress_reporter()` turns those events into the `phase`/`phase_detail`/`percent` fields carried on the job and streamed over SSE; `_MONITOR_STAGES` maps operation tags onto slices of one monotonic bar. Reading frames is counted per frame (each frame's read ends with its own 100% line), so the UI can say "7 of 14" truthfully. If a future ImageMagick stops matching that format nothing breaks — no events parse and the bar simply stays on its indeterminate state. `run_with_progress()` raises exactly what `subprocess.run(check=True)` would, keeping timeout/error handling unchanged, and strips progress lines out of the stderr used for user-facing error messages
